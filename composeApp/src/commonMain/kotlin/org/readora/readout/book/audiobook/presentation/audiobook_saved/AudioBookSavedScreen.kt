...
import org.readora.readout.book.audiobook.presentation.audiobook_home.components.AudioBookGridItem
...
@Composable
fun AudioBookSavedScreenRoot(
    viewModel: AudioBookSavedViewModel = koinViewModel(),
    ...
    AudioBookSavedScreen(
        state = state,
        innerPadding = innerPadding,
        windowSize = windowSize,
        onAction = { action ->
            when (action) {
                is AudioBookSavedAction.OnBookClick -> onBookClick(action.book)
                is AudioBookSavedAction.OnBackClick -> onBackClick()
            }
        }
    )
}
...
@Composable
private fun AudioBookSavedScreen(
    state: AudioBookSavedState,
    innerPadding: PaddingValues,
    windowSize: WindowSizes,
    onAction: (AudioBookSavedAction) -> Unit
) {
        ...
        Scaffold(
            ...
            topBar = {
                TopAppBar(
                    ...
                    navigationIcon = {
                        IconButton(onClick = {
                            onAction(AudioBookSavedAction.OnBackClick)
                        }) {
                            Icon(
                ...
                if (state.savedBooks.isNotEmpty()) {
                    items(
                        ...
                        AudioBookGridItem(
                            book = book,
                            onClick = {
                                onAction(AudioBookSavedAction.OnBookClick(book))
                            }
                        )
                    }
                } else {
